...
public class NewHook : MonoBehaviour
{
    ...
    private void Hook(InputAction.CallbackContext context)
    {
        ...
        else
        {
            ...
            initialAngularVelocity = _rb.angularVelocity;
        }
    }

    private void ApplyForceDuringSwing()
    {
        // Ensure the player is still hooked onto the pendulum
        if (isHooking && pendulum != null)
        {
            // Calculate the relative velocity between the player and the pendulum's forward direction
            Vector3 relativeVelocity = Vector3.Project(_rb.velocity - pendulum.transform.forward * Vector3.Dot(_rb.velocity, pendulum.transform.forward), pendulum.transform.forward);
            _rb.AddForce(relativeVelocity, ForceMode.Acceleration);
        }
    }
